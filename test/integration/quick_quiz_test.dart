...
void main() {
  group('Quick Quiz Tests', () {
    ...
    test('Quick Quiz loads 14 random questions for SBF-Binnen-Segeln', () async {
      // Select SBF-Binnen-Segeln course
      if (provider.manifest != null && 
          provider.manifest!.courses.containsKey('sbf-binnen-segeln')) {
        final sbfBinnenSegelnCourse = provider.manifest!.courses['sbf-binnen-segeln']!;
        provider.setSelectedCourse('sbf-binnen-segeln', sbfBinnenSegelnCourse);
    ...
    test('Quick Quiz falls back to legacy format when no course selected', () async {
    ...
    test('Session is properly started after loading quick quiz', () async {
      if (provider.manifest != null && 
        ...
        for (int i = 0; i < 14; i++) {
          expect(provider.currentSession!.questionIds[i], 
